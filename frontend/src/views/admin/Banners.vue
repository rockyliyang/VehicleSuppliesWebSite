    ...
    async fetchBanners() {
      ...
      try {
        ...
        if (response.success) {
          ...
          this.$messageHandler.showError(response.message, 'admin.banners.error.fetchFailed')
        }
      } catch (error) {
        console.error('获取Banner列表失败:', error)
        this.$messageHandler.showError(error, 'admin.banners.error.fetchFailed')
      } finally {
        this.loading = false
      }
    },
    ...
    handleDelete(row) {
      this.$confirm('确认删除该Banner吗?', '提示', {
        ...
        try {
          ...
          if (response.success) {
            this.$messageHandler.showSuccess('删除成功', 'banner.success.deleteSuccess')
            this.fetchBanners()
          } else {
            this.$messageHandler.showError(response.message, 'admin.banners.error.deleteFailed')
          }
        } catch (error) {
          console.error('删除Banner失败:', error)
          this.$messageHandler.showError(error, 'admin.banners.error.deleteFailed')
        }
      }).catch(() => {})
    },
      ...
      this.$refs.bannerForm.validate(async valid => {
        if (valid) {
          ...
          try {
            ...
            if (response.success) {
              this.$messageHandler.showSuccess(this.dialogStatus === 'create' ? '添加成功' : '更新成功', this.dialogStatus === 'create' ? 'banner.success.createSuccess' : 'banner.success.updateSuccess')
              this.dialogVisible = false
              this.fetchBanners()
            } else {
              this.$messageHandler.showError(response.message, this.dialogStatus === 'create' ? 'admin.banners.error.createFailed' : 'admin.banners.error.updateFailed')
            }
          } catch (error) {
            console.error(this.dialogStatus === 'create' ? '添加Banner失败:' : '更新Banner失败:', error)
            this.$messageHandler.showError(error, this.dialogStatus === 'create' ? 'admin.banners.error.createFailed' : 'admin.banners.error.updateFailed')
          } finally {
