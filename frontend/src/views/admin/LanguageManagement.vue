  ...
  methods: {
    ...
    async loadTranslations() {
      ...
      try {
        ...
        if (response.success) {
          ...
          ElMessage.error(response.message || '加载翻译失败')
        }
      } catch (error) {
    ...
    async submitTranslation() {
      ...
      await this.$refs.translationFormRef.validate(async (valid) => {
        if (valid) {
          try {
            const response = await this.$api.post('/language/admin/translations', this.translationForm)
            if (response.success) {
              ElMessage.success('添加翻译成功')
              this.addDialogVisible = false
              this.loadTranslations() // 重新加载列表
            } else {
              ElMessage.error(response.message || '添加翻译失败')
            }
          } catch (error) {
        ...
        if (valid) {
          try {
            ...
            if (response.success) {
              ...
              ElMessage.error(response.message || '更新翻译失败')
            }
          } catch (error) {
      ...
      ElMessageBox.confirm(
        ...
        try {
          ...
          if (response.success) {
            ...
            ElMessage.error(response.message || '删除翻译失败')
          }
        } catch (error) {
