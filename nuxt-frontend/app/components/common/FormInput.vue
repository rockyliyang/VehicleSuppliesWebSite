...
export default {
  ...
  data() {
    return {
      // 在组件创建时生成一次随机name，避免SSR水合不匹配
      randomName: 'field_' + Math.random().toString(36).substr(2, 9)
    }
  },
  computed: {
    inputValue: {
    ...
    computedPrefixIcon() {
      const icon = this.$props['prefix-icon'] || this.prefixIcon;
      return icon ? markRaw(icon) : null;
    },
    computedSuffixIcon() {
      const icon = this.$props['suffix-icon'] || this.suffixIcon;
      return icon ? markRaw(icon) : null;
    },
    computedShowPassword() {
      return this.$props['show-password'] || this.showPassword;
    }
  },
  methods: {
    handleInput(value) {
