  ...
  methods: {
    ...
    getPriceRangeDisplay(item) {
      if (item.unit_price) {
        ...
        const formatPrice = this.$store.formatPrice;
    ...
    handleCheckout() {
      ...
      if (this.inquiry && this.inquiry.items.length > 0) {
        ...
        const cartItems = this.inquiry.items.map(item => ({
          ...
          image_url: item.imageUrl || item.image_url || '/images/default-image.svg',
      ...
      try {
        ...
        if (response.success) {
          ...
          const newItem = {
            ...
            imageUrl: response.data.image_url || '/images/default-image.svg',
