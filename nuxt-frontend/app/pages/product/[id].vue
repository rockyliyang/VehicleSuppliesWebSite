        ...
        <div class="product-detail-content">
          ...
          <div class="product-info-block">
            ...
            <div class="product-price">
              ...
              <div v-if="product.price_ranges && product.price_ranges.length > 0" class="price-ranges">
                <div v-for="(range, index) in product.price_ranges" :key="index" class="price-range-item">
                  ...
                  <div class="range-price">{{ $store.formatPrice(range.price) }}</div>
                </div>
              </div>
              <!-- 如果没有阶梯价格，显示普通价格 -->
            ...
            <div class="product-short-desc">
              <p style="white-space: pre-wrap;">{{ product.short_description}}</p>
            </div>
          ...
          <div class="buy-together-summary">
            ...
            <div class="selected-products-info">
              <div v-if="currentProductSelected" class="product-info-item">
                ...
                <span class="product-price">{{ $store.formatPrice(getCurrentProductPrice()) }}</span>
              </div>
              <template v-for="item in buyTogetherProducts" :key="item.id">
                <div v-if="item.selected" class="product-info-item">
                  <span class="product-name">{{ item.name }}</span>
                  <span class="product-price">{{ $store.formatPrice(item.price) }}</span>
                </div>
              </template>
            </div>
...
import { handleChatNow, addProductToCart, handleLoginSuccess,  addBrowsingHistory, checkFavoriteStatus, getMinQuantityFromPriceRanges } from '~/utils/productUtils';
...
import { FullScreen, Close, Star } from '@element-plus/icons-vue';
...
export default {
  ...
  async setup() {
    ...
    try {
      console.log('setup SSR data fetching is :',productId);

      // 使用useLazyAsyncData进行SSR数据获取
      const { data: ssrData } = await useAsyncData(`product-${productId}`, async () => {
        // 并行获取产品数据和分类数据
        ...
        if (!productResponse.success || !productResponse.data ) {
          console.log('productResponse data is', productResponse.data);
        ...
        // 获取一起购买的产品
        let buyTogetherProducts = [];
        try {
          const buyTogetherResponse = await $api.get(`products/${productId}/buy-together`);
          buyTogetherProducts = (buyTogetherResponse.data || []).map(item => ({
            ...item,
            selected: false
          }));
        } catch (err) {
          console.warn('Failed to fetch buy together products:', err);
        }
      ...
      // 设置页面SEO元数据
      const product = ssrData.value?.product;
      if (product) {
        useHead({
          title: `${product.name} - Auto Ease TechX` || 'Product Detail',
          meta: [
            { name: 'description', content: product.short_description || product.name },
            { name: 'keywords', content: product.name },
            { property: 'og:title', content: product.name },
            { property: 'og:description', content: product.short_description || product.name },
            { property: 'og:image', content: product.thumbnail_url },
  ...
  data() {
    return {
      ...
      buyNowDialogVisible: false
    }
  },
  computed: {
    $store() {
      return this.$nuxt.$store;
    },
    $t() {
      return this.$nuxt.$t;
    },
    ...
    showBuyNowDialog: {
      get() {
        return this.buyNowDialogVisible || false;
      },
      set(value) {
        this.buyNowDialogVisible = value;
      }
    }
  },
  watch: {
    ...
    '$route.params.id'(newId) {
      ...
      const parsedId = newId && !isNaN(parseInt(newId)) ? parseInt(newId) : null;
      if (!parsedId) {
        console.error('Invalid product ID in route change:', newId);
        return;
      }
  ...
  created() {
    ...
    this.productId = routeId && !isNaN(parseInt(routeId)) ? parseInt(routeId) : null;
    if (!this.productId) {
      console.error('Invalid product ID:', routeId);
      throw createError({ statusCode: 404, statusMessage: 'Invalid product ID' });
    }
  ...
  async mounted() {
    ...
    window.scrollTo(0, 0);
    if (!this.categories) {
      console.log('SSR data not available, fetching categor from client side');
      await this.fetchCategories();
    }    
    // 检查SSR数据是否正常，如果不正常则重新获取
    if (!this.product || !this.product.id) {
      console.log('SSR data not available, fetching from client side');
      await this.fetchProduct();
    } else {
      // SSR数据正常，但仍需要执行一些客户端特定的逻辑
      if (this.isLoggedIn) {
        await addBrowsingHistory(this.product.id, this);
        await this.checkFavoriteStatusLocal();
      }
      //console.log('SSR data normal, product is',this.product);
      // 设置默认活动媒体
      this.$nextTick(() => {
        if (this.galleryImages && this.galleryImages.length > 0) {
          this.setActiveMedia(this.galleryImages[0]);
    ...
    async fetchProduct() {
      try {
        if (!this.productId || isNaN(this.productId)) {
          console.error('Invalid productId for fetchProduct:', this.productId);
    ...
    // 处理登录成功
     async handleLoginSuccess() {
       ...
       if (this.pendingAction && this.product) {
         ...
         if (this.pendingAction === 'buyNow') {
            await this.openBuyNowDialog();
         } else if (this.pendingAction === 'sendAllToInquiry') {
...
<style lang="scss">
@use '@/assets/styles/_quill-global.scss' as *;
</style>
...
.buy-now-btn {
  ...
  &:hover:not(:disabled) {
    background: color.adjust($success-color, $lightness: -10%) !important;
    border-color: color.adjust($success-color, $lightness: -10%) !important;
  }
}
...
.send-all-inquiry-btn {
  ...
  &:hover:not(:disabled) {
    background-color: color.adjust($success-color, $lightness: -10%) !important;
    border-color: color.adjust($success-color, $lightness: -10%) !important;
  }
