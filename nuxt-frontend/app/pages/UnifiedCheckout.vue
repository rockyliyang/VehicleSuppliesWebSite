          ...
          <div class="desktop-only">
            <el-table :data="orderItems" class="order-table">
              ...
              <el-table-column :label="$t('checkout.unitPrice') || '单价'" width="120" align="right">
                <template #default="{row}">{{ $store.formatPrice(row.price) }}</template>
              </el-table-column>
              <el-table-column :label="$t('checkout.subtotal') || '小计'" width="120" align="right">
                <template #default="{row}">
                  <span class="subtotal-price">{{ $store.formatPrice(row.price * row.quantity) }}</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div class="order-total">
            <div class="total-container">
              <div class="total-row">
                ...
                <span class="price">{{ $store.formatPrice(orderTotal) }}</span>
              </div>
              <div class="total-row">
                <span>{{ $t('checkout.shippingFee') || '运费' }}:</span>
                <span class="price">{{ $store.formatPrice(shippingFee) }}</span>
              </div>
              <div class="total-row grand-total">
                <span>{{ $t('checkout.total') || '总计' }}:</span>
                <span class="total-price">{{ $store.formatPrice(orderTotal + shippingFee) }}</span>
              </div>
            </div>
          </div>
        </section>
...
export default {
  ...
  computed: {
    ...
    countries() {
      const allCountries = this.$store.location.countries || [];
