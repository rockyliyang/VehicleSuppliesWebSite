...
export default {
  ...
  methods: {
    ...
    submitForm() {
      const { $api, $messageHandler, $t } = useNuxtApp();
      this.$refs.formRef.validate(async valid => {
        if (!valid) return;
        this.loading = true;
        try {
          const res = await $api.postWithErrorHandler('/users/forgot-password', {
            email: this.form.email,
            captcha: this.form.captcha
          });
          if (res.success) {
            $messageHandler.showSuccessAlert(
              $t('forgotPassword.success.message') || '重置密码链接已发送到您的邮箱，请查收！',
            ...
            $messageHandler.showError(res.message, 'forgotPassword.error.failed');
            this.refreshCaptcha();
          }
        } catch (e) {
          $messageHandler.showError(e, 'forgotPassword.error.failed');
          this.refreshCaptcha();
        } finally {
          this.loading = false;
        }
