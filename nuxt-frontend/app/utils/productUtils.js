 ...
 */
export async function addProductToCart(product, context, showLoginDialog, setAddingState, quantity = 1) {
  // 检查用户是否已登录
  ...
  try {
    // 使用现有的购物车工具函数
    const { handleAddToCart } = await import('./cartUtils');
...
export async function addBrowsingHistory(product, context) {
  if (!context.$store.auth.isLoggedIn || !product.id) return;
...
export async function checkFavoriteStatus(product, context) {
  if (!context.$store.auth.isLoggedIn || !product.id) return false;
