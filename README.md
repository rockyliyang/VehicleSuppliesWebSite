...
## 账户设置
在账户设置页面，用户可以看到自己的账号信息，用户名，邮箱，手机号，货币，用户只是可以修改手机号。
在账户设置页面，有地址管理，收藏管理，浏览记录
...
在购物车页面下方有一个询价功能，询价功能里面包含多个询价单，用户最多添加10个询价单，可以删除之前的询价，每个询价功能包含若干个商品的询价。在UI上可以用不同的方式体现询价单，可以用顶部tab页，也可以用侧边的tab 页，在某个tab页，包含每个询价单的所有功能。

在询价单界面，用户可以通过聊天功能跟和业务员讨论这个商品，如果业务员在后台修改报价单的商品报价，购物车的价格将会用询价单的报价覆盖产品的原来价格，如果用户没有通过询价功能讨论价格，购物车商品的价格是商品的报价，用户看到购物车的价格变化后，就可以直接下单

用户新加一个询价后，里面的商品是空的。在购物车的商品右侧，有一个询价按钮，如果这个时候商品没有在询价单里面，当前没有激活任何一个询价单，就新加一个询价单，自动把商品添加到询价单里面，如果已经激活一个询价单，则往这个询价单添加商品，如果商品已经在某一个询价单，则询价按钮不能点击，鼠标放到询价按钮上提示商品已经在某个询价单里。

### 询价功能业务员端
后台管理页面有一个询价功能管理界面，业务进去首先看到按他下面管理的用户列表和每个用户是否有未读消息，点击进一个用户，看到用户所有的询价单列表，每个询价单上是否有未读消息，每个询价单可以看到每个商品可以设置他的期望价格，购买个数，和历史聊天消息，可以在每个询价单界面给用户发消息。如果最后同意用户提出的价格，业务员修改询价单里面的报价，购物车上的价格也同时更新成新的报价

### 用户询价单管理
用户通过询价管理菜单进入询价单管理界面，用户可以查看自己的所有询价单，可以新建询价单，可以删除自己的询价单，可以修改自己的询价单，可以查看自己的询价单的历史聊天记录。

在电脑端，左侧是询价单列表按已经支付的询价单和未支付的询价单分类，在列表上有新建询价单功能。点击每个询价单，会进入询价单详情页，在详情页可以查看询价单的所有商品，商品的期望价格，购买个数，和历史聊天记录。 点击新建询价按钮将会新建一个空白的询价单，然后可以询价单商品列表里面添加商品。 

在手机端，询价单管理界面会是3个功能，分别是新建询价单，查看已经支付询价单列表，查看未支付询价单详情。

点击新建询价单，会进入新建询价单页面，在新建询价单页面可以添加商品，商品的期望价格，购买个数。

点击查看已经支付询价单列表，会进入已经支付询价单列表页面，在这个页面可以查看所有已经支付的询价单，点击每个询价单，会进入询价单详情页，在详情页可以查看询价单的所有商品，商品的期望价格，购买个数，和历史聊天记录。但是Checkout 按钮是Disable 的。

点击查看未支付询价单详情，会进入未支付询价单详情页面，在这个页面可以查看所有未支付的询价单，点击每个询价单，会进入询价单详情页，在详情页可以查看询价单的所有商品，商品的期望价格，购买个数，和历史聊天记录。




## Chrome 插件
Chrome 插件的功能是在负责在1688 网站上导出商品数据，当用户选品后进入商品的详情页面，用户可以点击插件的图标，插件会获取商品的详细信息，包括商品名称，商品的主图，轮播图，商品的详情，包括详情页里面的图片都转换成，我们的产品信息，通过调用我们的产品的API 接口，把商品信息保存到我们的数据库里面。

## 产品价格范围
我们要实现阶梯价格支持，我们可以设置当用户购买同一个商品，数量不同的时候价格不一样。 	我们先要 添加一个product_price_range 表， 表的字段包括数量范围的上限，下限，价格，商品Id. 后台需要在创建，修改商品的api支持往这个表填入数据,, 提交修改信息的时候把多个价格范围作为数组传到后端，在前端先检查在价格范围之间是否有间隔，如果有间隔，比如两个范围1-10,12-20，就要报错，最初的范围的下限是从1开始，最后的范围的上限是无穷大。 在返回商品信息的时候使用数组返回这个表的数据，展现的时候可以把多个范围组成一个字符串，不用原来的price 字段，需要提供另外一个版本的format price函数在。这个价格只是在商品展现的时候，在购物车和Checkout里面的价格不是价格范围，而是具体的价格。在用户修改订购数量的时候，按照用户选择的购买商品的数量，匹配对应的价格区间动态设置价格。 实现这个功能请注意前后代码规范，注意数据规范

### 运费计算
现在我们要在添加运费计算功能，
1. 我们要加载默认物流公司的运费范围数据和运费参数，接口请看. 
2. 定位到运费范围，根据shipping information 用户选择的国家，如果在运费范围里有这个国家id的特定运费范围则使用这个国家的运费范围，
否则按照这个国家的标签去寻找这个运费范围，因为这个国家的标签有多个，我们需要用物流公司名字取定位标签，比如物流公司是ShunFen, 
那个我们就匹配ShunFen开头的标签，如果我们匹配到一个，比如ShunFen_Euro,  则使用这个标签去寻找运费范围，如果找不到，则用下一个匹配到的ShunFen 标签， 
直到找不到。 最后都匹配不到，则用这个公司默认的运费范围。 
3. 定位运费参数，方法和第二步类似，通过国家id和tagid, 在shippingfee_fatctor表里找到对应的运费参数包括首重重量，首重费用，抛比系数, 附加费，附加费2，折扣，总费用
4. 用第二步和第三部获得的数据开始计算，基本逻辑如下

首先用产品的长*宽*高/抛比系数获得体积重量, 并且把多个商品的体积重量加在一起，然后和产品的实际重量比较，谁大用谁做计算重量。再用基本公式计算费用，
